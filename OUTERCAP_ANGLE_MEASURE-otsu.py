...
LOWER_BORE_UP_BOUND = 200 
LOWER_BORE_LOW_BOUND = 120

TOP_BORE_THRESHOLD = 30 #0.3 # was .25
LOWER_BORE_THRESHOLD = 20 #0.3 #0.2 was over sat
...
def create_composite_image(ledge_path, ax1, specs): 
    ...
    try: 
        check_rot_points_tightness(cap_xs, cap_ys)
        check_rot_points_tightness(bore_xs, bore_ys)
...
def get_circle_locations(image, name, outputs, specs):
    print("\tanalyzing image: " + str(name))
    try:
        
#        plt.hist(image[:,:,0].ravel(), color='r')
#        plt.show()
#        plt.hist(image[:,:,1].ravel(), color='g')
#        plt.show()
#        plt.hist(image[:,:,2].ravel(), color='b')
#        plt.show()
#        plt.imshow(image[:,:,0])
#        plt.show()
#        plt.imshow(image[:,:,1])
...
#        fig, ax = filters.try_all_threshold(grey_smooth, figsize=(10,8), verbose=False)
#        plt.show()
...
#        plt.show()
#        
        # blue seems to have a pretty good threshold val...
        #summed = image[:,:,2]
#        print('saovola')
#        saovola = grey > filters.threshold_sauvola(grey_smooth, window_size=151)
#        print('done')
#
#        plt.imshow(saovola, cmap=plt.cm.gray)
#        plt.axis('off')
#        plt.show()
        
        #I don't think smoothing is actually valuable here 
        #smoothed = filters.gaussian(summed, 5)       
        
        #mask = cutoff(summed, specs)
#        plt.imshow(mask)
#        plt.show()
